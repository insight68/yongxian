...
function filterLogsByRange(logs, rangeStart, rangeEnd) {
...
function renderSessionSummary(session, filteredUsage, filteredLogs) {
...
function computeFilteredUsage(baseUsage, points, rangeStart, rangeEnd) {
    const lo = Math.min(rangeStart, rangeEnd);
    const hi = Math.max(rangeStart, rangeEnd);
...
function renderSessionDetailPanel(session, timeSeries, timeSeriesLoading, timeSeriesMode, onTimeSeriesModeChange, timeSeriesBreakdownMode, onTimeSeriesBreakdownChange, timeSeriesCursorStart, timeSeriesCursorEnd, onTimeSeriesCursorRangeChange, startDate, endDate, selectedDays, sessionLogs, sessionLogsLoading, sessionLogsExpanded, onToggleSessionLogsExpanded, logFilters, onLogFilterRolesChange, onLogFilterToolsChange, onLogFilterHasToolsChange, onLogFilterQueryChange, onLogFilterClear, contextExpanded, onToggleContextExpanded, onClose) {
    ...
    const filteredUsage = timeSeriesCursorStart !== null && timeSeriesCursorEnd !== null && timeSeries?.points && usage
    ...
    <div class="card session-detail-panel">
      ...
      <div class="session-detail-content">
        ${renderSessionSummary(session, filteredUsage, timeSeriesCursorStart != null && timeSeriesCursorEnd != null && sessionLogs
        ...
        <div class="session-detail-bottom">
          ${renderSessionLogsCompact(sessionLogs, sessionLogsLoading, sessionLogsExpanded, onToggleSessionLogsExpanded, logFilters, onLogFilterRolesChange, onLogFilterToolsChange, onLogFilterHasToolsChange, onLogFilterQueryChange, onLogFilterClear, hasRange ? timeSeriesCursorStart : null, hasRange ? timeSeriesCursorEnd : null)}
...
function renderTimeSeriesCompact(timeSeries, loading, mode, onModeChange, breakdownMode, onBreakdownChange, startDate, endDate, selectedDays, cursorStart, cursorEnd, onCursorRangeChange) {
    if (loading) {
        return html `
    ...
    const barTotals = points.map((p) => isCumulative
        ? p.cumulativeTokens
        : breakdownByType
...
function renderContextPanel(contextWeight, usage, expanded, onToggleExpanded) {
    if (!contextWeight) {
        return html `
    ...
    const hasMore = skillsList.length > defaultLimit ||
...
function renderSessionLogsCompact(logs, loading, expandedAll, onToggleExpandedAll, filters, onFilterRolesChange, onFilterToolsChange, onFilterHasToolsChange, onFilterQueryChange, onFilterClear, cursorStart, cursorEnd) {
    if (loading) {
        return html `
    ...
    const toolOptions = Array.from(new Set(entries.flatMap((entry) => entry.toolInfo.tools.map(([name]) => name)))).toSorted((a, b) => a.localeCompare(b));
    ...
    <div class="session-logs-compact">
      ...
      <div class="usage-filters-inline" style="margin: 10px 12px;">
        <select
          multiple
          size="4"
          @change=${(event) => onFilterRolesChange(Array.from(event.target.selectedOptions).map((option) => option.value))}
        >
        ...
        <select
          multiple
          size="4"
          @change=${(event) => onFilterToolsChange(Array.from(event.target.selectedOptions).map((option) => option.value))}
        >
          ${toolOptions.map((tool) => html `<option value=${tool} ?selected=${toolSelected.has(tool)}>${tool}</option>`)}
        </select>
        <label class="usage-filters-inline" style="gap: 6px;">
          <input
            type="checkbox"
            .checked=${filters.hasTools}
            @change=${(event) => onFilterHasToolsChange(event.target.checked)}
          />
          Has tools
        </label>
        <input
          ...
          .value=${filters.query}
          @input=${(event) => onFilterQueryChange(event.target.value)}
        />
      ...
      <div class="session-logs-list">
        ...
        const roleLabel = log.role === "user" ? "You" : log.role === "assistant" ? "Assistant" : "Tool";
        return html `
            ...
            ? html `
                    <details class="session-log-tools" ?open=${expandedAll}>
                      ...
                      <div class="session-log-tools-list">
                        ${toolInfo.tools.map(([name, count]) => html `
