...
export class UsageView extends LitElement {
  ...
  @state()
  totals: UsageTotals | null = null;

  @state()
  aggregates: UsageAggregates | null = null;
  ...
  @state()
  dailyChartMode: "tokens" | "cost" = "cost";
  ...
  @state()
  sessionsTab: "all" | "recent" = "all";

  @state()
  sessionSort: "date" | "tokens" | "cost" = "date";

  @state()
  sessionSortDir: "asc" | "desc" = "desc";

  @state()
  visibleColumns: UsageColumnId[] = ["date", "agent", "channel", "model", "tokens", "cost"];

  @state()
  timeSeries: Array<{ timestamp: number; input: number; output: number; cost: number }> = [];

  @state()
  timeSeriesLoading = false;

  @state()
  timeSeriesMode: "tokens" | "cost" = "tokens";

  @state()
  timeSeriesBreakdownMode: "input" | "output" = "input";

  @state()
  timeSeriesCursorStart: number | null = null;

  @state()
  timeSeriesCursorEnd: number | null = null;
  ...
  @state()
  logFilterHasTools: boolean | null = null;
  ...
  render() {
    const props: UsageProps = {
      ...
      onStartDateChange: (v: string) => (this.startDate = v),
      onEndDateChange: (v: string) => (this.endDate = v),
      onSelectSession: (keys: string[]) => (this.selectedSessions = keys),
      ...
      onQueryDraftChange: (v: string) => (this.queryDraft = v),
      onApplyQuery: () => (this.query = this.queryDraft),
      onClearQuery: () => {
        this.query = "";
        this.queryDraft = "";
      },
      onChartModeChange: (v: "tokens" | "cost") => (this.chartMode = v),
      onDailyChartModeChange: (v: "tokens" | "cost") => (this.dailyChartMode = v),
      onToggleHeaderPinned: () => (this.headerPinned = !this.headerPinned),
      onTimeZoneChange: (v: "local" | "utc") => (this.timeZone = v),
      onRefresh: () => {
      ...
      onSessionsTabChange: (v: "all" | "recent") => (this.sessionsTab = v),
      onSessionSortChange: (v: "date" | "tokens" | "cost") => (this.sessionSort = v),
      onSessionSortDirChange: (v: "asc" | "desc") => (this.sessionSortDir = v),
      onTimeSeriesModeChange: (v: "tokens" | "cost") => (this.timeSeriesMode = v),
      onTimeSeriesBreakdownChange: (v: "input" | "output") => (this.timeSeriesBreakdownMode = v),
      onTimeSeriesCursorRangeChange: (start: number | null, end: number | null) => {
      ...
      onLogFilterRolesChange: (roles: SessionLogRole[]) => (this.logFilterRoles = roles),
      onLogFilterToolsChange: (tools: string[]) => (this.logFilterTools = tools),
      onLogFilterHasToolsChange: (hasTools: boolean | null) => (this.logFilterHasTools = hasTools),
      onLogFilterQueryChange: (query: string) => (this.logFilterQuery = query),
      onLogFilterClear: () => {
        this.logFilterRoles = [];
        this.logFilterTools = [];
