...
import { buildAggregatesFromSessions, buildPeakErrorHours, buildUsageInsightStats, formatCost, formatIsoDate, formatTokens, getZonedHour, renderUsageMosaic, setToHourEnd, } from "./usage-metrics.ts";
import { addQueryToken, applySuggestionToQuery, buildDailyCsv, buildQuerySuggestions, buildSessionsCsv, downloadTextFile, normalizeQueryText, removeQueryToken, setQueryTokensForKey, } from "./usage-query.ts";
import { renderEmptyDetailState, renderSessionDetailPanel } from "./usage-render-details.ts";
import { renderCostBreakdownCompact, renderDailyChartCompact, renderFilterChips, renderSessionsCard, renderUsageInsights, } from "./usage-render-overview.ts";
...
function addUsageTotals(acc, usage) {
...
export function renderUsage(props) {
    ...
    const dayFilteredSessions = props.selectedDays.length > 0
        ...
        : sortedSessions;
    const sessionTouchesHours = (session, hours) => {
        if (hours.length === 0) {
            return true;
        }
    ...
    const hourFilteredSessions = props.selectedHours.length > 0
    ...
    const querySuggestions = buildQuerySuggestions(props.queryDraft, sortedSessions, props.aggregates);
    const queryTerms = extractQueryTerms(props.query);
    const selectedValuesFor = (key) => {
    ...
    const toolOptions = unique(props.aggregates?.tools.tools.map((tool) => tool.name) ?? []).slice(0, 12);
    // Get first selected session for detail view (timeseries, logs)
    const primarySelectedEntry = props.selectedSessions.length === 1
    ...
    const computeSessionTotals = (sessions) => {
        return sessions.reduce((acc, s) => (s.usage ? addUsageTotals(acc, s.usage) : acc), createEmptyUsageTotals());
    };
    // Compute totals from daily data for selected days (more accurate than session totals)
    const computeDailyTotals = (days) => {
    ...
    let displayTotals;
    let displaySessionCount;
    const totalSessions = sortedSessions.length;
    if (props.selectedSessions.length > 0) {
        // Sessions selected - compute totals from selected sessions
        const selectedSessionEntries = filteredSessions.filter((s) => props.selectedSessions.includes(s.key));
    ...
    const aggregateSessions = props.selectedSessions.length > 0
    ...
    const filteredDaily = props.selectedSessions.length > 0
        ? (() => {
            const selectedEntries = filteredSessions.filter((s) => props.selectedSessions.includes(s.key));
    ...
    const renderFilterSelect = (key, label, options) => {
        ...
        const allSelected = options.length > 0 && options.every((value) => selectedSet.has(normalizeQueryText(value)));
        ...
        <div class="usage-filter-popover">
          ...
          <div class="usage-filter-options">
            ...
            return html `
                ...
                props.onQueryDraftChange(target.checked
    ...
    <section class="card usage-header ${props.headerPinned ? "pinned" : ""}">
            ...
            <div class="usage-export-popover">
              <div class="usage-export-list">
                <button
                  ...
                  @click=${() => downloadTextFile(`openclaw-usage-sessions-${exportStamp}.csv`, buildSessionsCsv(filteredSessions), "text/csv")}
                ...
                <button
                  class="usage-export-item"
                  @click=${() => downloadTextFile(`openclaw-usage-daily-${exportStamp}.csv`, buildDailyCsv(filteredDaily), "text/csv")}
                ...
                <button
                  class="usage-export-item"
                  @click=${() => downloadTextFile(`openclaw-usage-${exportStamp}.json`, JSON.stringify({
      ...
      <div class="usage-header-row">
        <div class="usage-controls">
          ...
          <input
            ...
            title="Start Date"
            @change=${(e) => props.onStartDateChange(e.target.value)}
          />
          <span style="color: var(--muted);">to</span>
          <input
            type="date"
            .value=${props.endDate}
            title="End Date"
            @change=${(e) => props.onEndDateChange(e.target.value)}
          />
          <select
            title="Time zone"
            .value=${props.timeZone}
            @change=${(e) => props.onTimeZoneChange(e.target.value)}
          >
      ...
      <div style="margin-top: 12px;">
          ...
          <input
            ...
            @input=${(e) => props.onQueryDraftChange(e.target.value)}
            @keydown=${(e) => {
        if (e.key === "Enter") {
            e.preventDefault();
            props.onApplyQuery();
        ...
        ? html `
            ...
            return html `
                      <span class="usage-query-chip">
                        ...
                        <button
                          title="Remove filter"
                          @click=${() => props.onQueryDraftChange(removeQueryToken(props.queryDraft, label))}
                        >
                          ×
                        </button>
                      </span>
                ...
                <div class="usage-query-suggestions">
                  ${querySuggestions.map((suggestion) => html `
                      <button
                        ...
                        @click=${() => props.onQueryDraftChange(applySuggestionToQuery(props.queryDraft, suggestion.value))}
                      >
                        ${suggestion.label}
                      </button>
                    `)}
    ...
    ${renderUsageInsights(displayTotals, activeAggregates, insightStats, hasMissingCost, buildPeakErrorHours(aggregateSessions, props.timeZone), displaySessionCount, totalSessions)}
    ...
    <div class="usage-grid">
      <div class="usage-grid-left">
        <div class="card usage-left-card">
          ${renderDailyChartCompact(filteredDaily, props.selectedDays, props.chartMode, props.dailyChartMode, props.onDailyChartModeChange, props.onSelectDay)}
    ...
    ${primarySelectedEntry
        ? renderSessionDetailPanel(primarySelectedEntry, props.timeSeries, props.timeSeriesLoading, props.timeSeriesMode, props.onTimeSeriesModeChange, props.timeSeriesBreakdownMode, props.onTimeSeriesBreakdownChange, props.timeSeriesCursorStart, props.timeSeriesCursorEnd, props.onTimeSeriesCursorRangeChange, props.startDate, props.endDate, props.selectedDays, props.sessionLogs, props.sessionLogsLoading, props.sessionLogsExpanded, props.onToggleSessionLogsExpanded, {
            roles: props.logFilterRoles,
            tools: props.logFilterTools,
            hasTools: props.logFilterHasTools,
            query: props.logFilterQuery,
        }, props.onLogFilterRolesChange, props.onLogFilterToolsChange, props.onLogFilterHasToolsChange, props.onLogFilterQueryChange, props.onLogFilterClear, props.contextExpanded, props.onToggleContextExpanded, props.onClearSessions)
        : renderEmptyDetailState()}
  `;
}
// Exposed for Playwright/Vitest browser unit tests.
/**
 * Usage View Component
 *
 * 使用情况页面组件 - 展示 Token 使用情况和成本分析
 */
let UsageView = class UsageView extends LitElement {
    constructor() {
        super(...arguments);
        this.loading = true;
        this.error = null;
        ...
        this.dailyChartMode = "cost";
        this.headerPinned = false;
        this.timeZone = "local";
        this.recentSessions = [];
        this.sessionsTab = "all";
        this.sessionSort = "date";
        this.sessionSortDir = "desc";
        this.visibleColumns = ["date", "agent", "channel", "model", "tokens", "cost"];
        this.timeSeries = [];
        this.timeSeriesLoading = false;
        this.timeSeriesMode = "tokens";
        this.timeSeriesBreakdownMode = "input";
        this.timeSeriesCursorStart = null;
        this.timeSeriesCursorEnd = null;
        this.sessionLogs = [];
        this.sessionLogsLoading = false;
        this.sessionLogsExpanded = false;
        this.logFilterRoles = [];
        this.logFilterTools = [];
        this.logFilterHasTools = null;
        this.logFilterQuery = "";
  ...
  `; }
    ...
    connectedCallback() {
        ...
        this.startDate = formatIsoDate(start);
        this.endDate = formatIsoDate(end);
        this.loading = false;
    }
    formatIsoDate(d) {
    ...
    render() {
        const props = {
            ...
            sessionsLimitReached: this.sessionsLimitReached,
            totals: this.totals,
            aggregates: this.aggregates,
            costDaily: this.costDaily,
            ...
            timeSeriesCursorStart: this.timeSeriesCursorStart,
            timeSeriesCursorEnd: this.timeSeriesCursorEnd,
            ...
            onStartDateChange: (v) => (this.startDate = v),
            onEndDateChange: (v) => (this.endDate = v),
            onSelectSession: (keys) => (this.selectedSessions = keys),
            onSelectDay: (day) => {
            ...
            onSelectHour: (hour) => {
                this.selectedHours = this.selectedHours.includes(hour)
                    ? this.selectedHours.filter((h) => h !== hour)
                    : [...this.selectedHours, hour];
            },
            onClearDays: () => (this.selectedDays = []),
            onClearHours: () => (this.selectedHours = []),
            onClearSessions: () => (this.selectedSessions = []),
            onClearFilters: () => {
                this.selectedDays = [];
                this.selectedHours = [];
                this.selectedSessions = [];
            ...
            onQueryDraftChange: (v) => (this.queryDraft = v),
            onApplyQuery: () => (this.query = this.queryDraft),
            onClearQuery: () => {
                this.query = "";
                this.queryDraft = "";
            },
            onChartModeChange: (v) => (this.chartMode = v),
            onDailyChartModeChange: (v) => (this.dailyChartMode = v),
            onToggleHeaderPinned: () => (this.headerPinned = !this.headerPinned),
            onTimeZoneChange: (v) => (this.timeZone = v),
            onRefresh: () => {
            ...
            onSessionsTabChange: (v) => (this.sessionsTab = v),
            onSessionSortChange: (v) => (this.sessionSort = v),
            onSessionSortDirChange: (v) => (this.sessionSortDir = v),
            onTimeSeriesModeChange: (v) => (this.timeSeriesMode = v),
            onTimeSeriesBreakdownChange: (v) => (this.timeSeriesBreakdownMode = v),
            onTimeSeriesCursorRangeChange: (start, end) => {
                this.timeSeriesCursorStart = start;
                this.timeSeriesCursorEnd = end;
            },
            onToggleSessionLogsExpanded: () => (this.sessionLogsExpanded = !this.sessionLogsExpanded),
            onLogFilterRolesChange: (roles) => (this.logFilterRoles = roles),
            onLogFilterToolsChange: (tools) => (this.logFilterTools = tools),
            onLogFilterHasToolsChange: (hasTools) => (this.logFilterHasTools = hasTools),
            onLogFilterQueryChange: (query) => (this.logFilterQuery = query),
            onLogFilterClear: () => {
                this.logFilterRoles = [];
                this.logFilterTools = [];
                this.logFilterHasTools = null;
        ...
        return renderUsage(props);
    }
};
__decorate([
    state()
], UsageView.prototype, "loading", void 0);
__decorate([
    state()
], UsageView.prototype, "error", void 0);
__decorate([
    state()
], UsageView.prototype, "startDate", void 0);
__decorate([
    state()
], UsageView.prototype, "endDate", void 0);
__decorate([
    state()
], UsageView.prototype, "sessions", void 0);
__decorate([
    state()
], UsageView.prototype, "sessionsLimitReached", void 0);
__decorate([
    state()
], UsageView.prototype, "totals", void 0);
__decorate([
    state()
], UsageView.prototype, "aggregates", void 0);
__decorate([
    state()
], UsageView.prototype, "costDaily", void 0);
__decorate([
    state()
], UsageView.prototype, "selectedSessions", void 0);
__decorate([
    state()
], UsageView.prototype, "selectedDays", void 0);
__decorate([
    state()
], UsageView.prototype, "selectedHours", void 0);
__decorate([
    state()
], UsageView.prototype, "query", void 0);
__decorate([
    state()
], UsageView.prototype, "queryDraft", void 0);
__decorate([
    state()
], UsageView.prototype, "chartMode", void 0);
__decorate([
    state()
], UsageView.prototype, "dailyChartMode", void 0);
__decorate([
    state()
], UsageView.prototype, "headerPinned", void 0);
__decorate([
    state()
], UsageView.prototype, "timeZone", void 0);
__decorate([
    state()
], UsageView.prototype, "recentSessions", void 0);
__decorate([
    state()
], UsageView.prototype, "sessionsTab", void 0);
__decorate([
    state()
], UsageView.prototype, "sessionSort", void 0);
__decorate([
    state()
], UsageView.prototype, "sessionSortDir", void 0);
__decorate([
    state()
], UsageView.prototype, "visibleColumns", void 0);
__decorate([
    state()
], UsageView.prototype, "timeSeries", void 0);
__decorate([
    state()
], UsageView.prototype, "timeSeriesLoading", void 0);
__decorate([
    state()
], UsageView.prototype, "timeSeriesMode", void 0);
__decorate([
    state()
], UsageView.prototype, "timeSeriesBreakdownMode", void 0);
__decorate([
    state()
], UsageView.prototype, "timeSeriesCursorStart", void 0);
__decorate([
    state()
], UsageView.prototype, "timeSeriesCursorEnd", void 0);
__decorate([
    state()
], UsageView.prototype, "sessionLogs", void 0);
__decorate([
    state()
], UsageView.prototype, "sessionLogsLoading", void 0);
__decorate([
    state()
], UsageView.prototype, "sessionLogsExpanded", void 0);
__decorate([
    state()
], UsageView.prototype, "logFilterRoles", void 0);
__decorate([
    state()
], UsageView.prototype, "logFilterTools", void 0);
__decorate([
    state()
], UsageView.prototype, "logFilterHasTools", void 0);
__decorate([
    state()
], UsageView.prototype, "logFilterQuery", void 0);
__decorate([
    state()
], UsageView.prototype, "contextExpanded", void 0);
UsageView = __decorate([
    customElement("openclaw-view-usage")
], UsageView);
export { UsageView };
