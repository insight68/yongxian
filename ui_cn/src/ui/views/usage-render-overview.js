...
function getCostBreakdown(totals) {
...
function renderFilterChips(selectedDays, selectedHours, selectedSessions, sessions, onClearDays, onClearHours, onClearSessions, onClearFilters) {
...
function renderDailyChartCompact(daily, selectedDays, chartMode, dailyChartMode, onDailyChartModeChange, onSelectDay) {
    if (!daily.length) {
        return html `
    ...
    <div class="daily-chart-compact">
      ...
      <div class="daily-chart">
        ...
        const segments = dailyChartMode === "by-type"
            ...
            : [];
        const breakdownLines = dailyChartMode === "by-type"
            ? isTokenMode
                ? [
        ...
        return html `
              <div
                ...
                @click=${(e) => onSelectDay(d.date, e.shiftKey)}
              >
...
function renderCostBreakdownCompact(totals, mode) {
...
function renderInsightList(title, items, emptyLabel) {
    return html `
...
function renderPeakErrorList(title, items, emptyLabel) {
    return html `
    <div class="usage-insight-card">
      <div class="usage-insight-title">${title}</div>
      ${items.length === 0
...
function renderUsageInsights(totals, aggregates, stats, showCostHint, errorHours, sessionCount, totalSessions) {
    if (!totals) {
        return nothing;
    }
...
function renderSessionsCard(sessions, selectedSessions, selectedDays, isTokenMode, sessionSort, sessionSortDir, recentSessions, sessionsTab, onSelectSession, onSessionSortChange, onSessionSortDirChange, onSessionsTabChange, visibleColumns, totalSessions, onClearSessions) {
    const showColumn = (id) => visibleColumns.includes(id);
    const formatSessionListLabel = (s) => {
    ...
    const totalErrors = sortedWithDir.reduce((sum, session) => sum + (session.usage?.messageCounts?.errors ?? 0), 0);
    const renderSessionBarRow = (s, isSelected) => {
      ...
      <div
        ...
        @click=${(e) => onSelectSession(s.key, e.shiftKey)}
        title="${s.key}"
      >
    ...
    <div class="card sessions-card">
      ...
      <div class="sessions-card-meta">
        ...
        <label class="sessions-sort">
          <span>Sort</span>
          <select
            @change=${(e) => onSessionSortChange(e.target.value)}
          >
...
export { renderCostBreakdownCompact, renderDailyChartCompact, renderFilterChips, renderInsightList, renderPeakErrorList, renderSessionsCard, renderUsageInsights, };
