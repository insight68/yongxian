...
// 硬编码为 ui_cn 目录
const uiDir = path.join(repoRoot, "ui_cn");

const WINDOWS_SHELL_EXTENSIONS = new Set([".cmd", ".bat", ".com"]);
const WINDOWS_UNSAFE_SHELL_ARG_PATTERN = /[\r\n"&|<>^%!]/;

function usage() {
  process.stderr.write("Usage: node scripts/ui_cn.js <install|dev|build|test> [...args]\n");
}
...
function shouldUseShellForCommand(cmd, platform = process.platform) {
  if (platform !== "win32") {
    return false;
  }
...
function assertSafeWindowsShellArgs(args, platform = process.platform) {
  if (platform !== "win32") {
    return;
  }
